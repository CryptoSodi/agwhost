...
package com.util.rtree
{
	...
	public class RTree
	{
		...
		private var rootNodeId:String                              = 'node0';
		private var size:int                                       = 0;

		// Enables creation of new nodes
		private var highestUsedNodeId:int                          = 0;

		// List of nearest rectangles. Use a member variable to
		// avoid recreating the object each time nearest() is called.
		private var nearestIds:Array                               = [];

		// Public methods
		/////////////////

		// Constructor
		public function RTree():void
		{

			this.maxNodeEntries = RTree.DEFAULT_MAX_NODE_ENTRIES
			this.minNodeEntries = RTree.DEFAULT_MIN_NODE_ENTRIES
			...
			{
				trace("Invalid MaxNodeEntries = " + this.maxNodeEntries + " Resetting to default value of " + RTree.DEFAULT_MAX_NODE_ENTRIES);
				this.maxNodeEntries = RTree.DEFAULT_MAX_NODE_ENTRIES;
			}
			...
			for (var i:int = 0; i < this.maxNodeEntries; i++)
				this.initialEntryStatus[i] = RTree.ENTRY_STATUS_UNASSIGNED;

			var root:RTreeNode = new RTreeNode(rootNodeId, 1, maxNodeEntries);
		...
		/**
		 * Adds a new RRectangle to the spatial index
		 *
		 * @param r  The RRectangle to add to the spatial index.
		 * @param id The ID of the RRectangle to add to the spatial index.
		 *           The result of adding more than one RRectangle with
		 *           the same ID is undefined.
		 */
		public function addRRectangle( r:RRectangle, id:String, level:int = 1 ):void
		{
			// I1 [Find position for new record] Invoke ChooseLeaf to select a 
			// leaf node L in which to place r
			var n:RTreeNode       = this.chooseNode(r, level);
			var newLeaf:RTreeNode = null;
			...
			var newNode:RTreeNode = this.adjustTree(n, newLeaf);

			// I4 [Grow tree taller] If node split propagation caused the root to 
			// split, create a new root whose children are the two resulting nodes.
			if (newNode != null)
			{
				var oldRootNodeId:String = this.rootNodeId;
				var oldRoot:RTreeNode = this.getNode(oldRootNodeId);

				this.rootNodeId = getNextNodeId();
				treeHeight++
				var root:RTreeNode = new RTreeNode(rootNodeId, treeHeight, maxNodeEntries);
				root.addEntry(newNode.mbr, newNode.nodeId);
				root.addEntry(oldRoot.mbr, oldRoot.nodeId);
				nodeMap[rootNodeId] = root;
			}

			if (RTree.INTERNAL_CONSISTENCY_CHECKING)
				this.checkConsistency(rootNodeId, treeHeight, null)

		}

		/**
		 * Deletes a RRectangle from the spatial index
		 *
		 * @param r  The RRectangle to delete from the spatial index
		 * @param id The ID of the RRectangle to delete from the spatial
		 *           index
		 *
		 * @return true  if the RRectangle was deleted
		 *         false if the RRectangle was not found, or the
		 *               RRectangle was found but with a different ID
		 */
		public function deleteRRectangle( r:RRectangle, id:String ):Boolean
		{
			...
			// the RRectangle we are searching for.
			//
			...
			var root:RTreeNode = this.getNode(rootNodeId);
		...
		/**
		 * Finds all rectangles that are nearest to the passed RRectangle
		 *
		 * @param p The point for which this method finds the nearest neighbours, as an Array of coordinates [x,y,z]
		 *
		 * @param distance The furthest distance away from the RRectangle to search. Rectangles further than this will not be found.
		 * This should be as small as possible to minimise the search time.
		 * Dafault Number.POSITIVE_INFINITY value guarantees that the nearest RRectangle is found, no matter how far away, although this will slow down the algorithm.
		 *
		...
		public function nearest( p:Array, furthestDistance:Number = Number.POSITIVE_INFINITY ):Array
		{
			...
			var rootNode:RTreeNode = getNode(rootNodeId);
		...
		/**
		 * Finds all rectangles that intersect the passed RRectangle.
		 *
		 * @param  r The RRectangle for which this method finds intersecting rectangles.
		 *
		 * @return An array with all RRectangle Ids than intersect the input RRectangle
		 */
		public function intersects( r:RRectangle ):Array
		{
		...
		/**
		 * Finds all rectangles contained by the passed RRectangle.
		 *
		 * @param r The RRectangle for which this method finds contained rectangles.
		 *
		 * @return An array with all RRectangle Ids than intersect the input RRectangle
		 */
		public function contains( r:RRectangle ):Array
		{

			var ret:Array = [];

			// find all rectangles in the tree that are contained by the passed RRectangle
			...
			{
				var n:RTreeNode    = this.getNode(this.parents[this.parents.length - 1]);
				var startIndex:int = this.parentsEntry[this.parentsEntry.length - 1] + 1;

				if (!n.isLeaf())
				{
					// go through every entry in the index node to check
					// if it intersects the passed RRectangle. If so, it 
				...
				} else
				{
					...
					// it is contained by the passed RRectangle
		 ...
		 */
		public function getBounds():RRectangle
		{

			var bounds:RRectangle = null

			var n:RTreeNode       = this.getNode(this.rootNodeId);
		...
		private function getNextNodeId():String
		{
			highestUsedNodeId++;
			return 'node' + highestUsedNodeId;
		}

		// Get a node object, given the ID of the node.
		private function getNode( index:String ):RTreeNode
		{
		 ...
		 */
		private function splitNode( n:RTreeNode, newRect:RRectangle, newId:String ):RTreeNode
		{
			...
			var newNode:RTreeNode = null;
			newNode = new RTreeNode(getNextNodeId(), n.level, this.maxNodeEntries);
			...
			while (n.entryCount + newNode.entryCount < this.maxNodeEntries + 1)
			{
				if (this.maxNodeEntries + 1 - newNode.entryCount == this.minNodeEntries)
				{
					...
					{
						if (this.entryStatus[i] == RTree.ENTRY_STATUS_UNASSIGNED)
						{
							this.entryStatus[i] = RTree.ENTRY_STATUS_ASSIGNED;
				...
				// next entry to assign. Add it to the group whose covering RRectangle 
			...
			if (RTree.INTERNAL_CONSISTENCY_CHECKING)
			{
				if (!n.mbr.equals(this.calculateMBR(n)))
					trace("Error: splitNode old node MBR wrong");
				if (!newNode.mbr.equals(this.calculateMBR(newNode)))
					trace("Error: splitNode new node MBR wrong");
			}
		 ...
		 */
		private function pickSeeds( n:RTreeNode, newRect:RRectangle, newId:String, newNode:RTreeNode ):void
		{

			// Find extreme rectangles along all dimension. Along each dimension,
			// find the entry whose RRectangle has the highest low side, and the one 
			...
			// the new RRectangle as well.
			n.mbr.add(newRect);

			for (var d:int = 0; d < RRectangle.DIMENSIONS; d++)
			{
				var tempHighestLow:Number   = newRect.min[d];
				var tempHighestLowIndex:int = -1; // -1 indicates the new RRectangle is the seed
				...
				for (var i:int = 0; i < n.entryCount; i++)
				{
					...
					// PS2 [Adjust for shape of the RRectangle cluster] Normalize the separations
			...
			} else
			{
				...
				// move the new RRectangle into the space vacated by the seed for the new node
			...
			entryStatus[lowestHighIndex] = RTree.ENTRY_STATUS_ASSIGNED;
			n.entryCount = 1;
		 ...
		 */
		private function pickNext( n:RTreeNode, newNode:RTreeNode ):int
		{
			...
			for (var i:int = 0; i < this.maxNodeEntries; i++)
			{
				if (this.entryStatus[i] == RTree.ENTRY_STATUS_UNASSIGNED)
				{
					...
					if (n.entries[i] == null)
					{
						trace("Error: RTreeNode " + n.nodeId + ", entry " + i + " is null");
					}
			...
			this.entryStatus[next] = RTree.ENTRY_STATUS_ASSIGNED;
		 ...
		 */
		private function recursiveNearest( p:Array, n:RTreeNode, nearestDistance:Number ):Number
		{
			for (var i:int = 0; i < n.entryCount; i++)
			{
				...
				{ // for index nodes, only go into them if they potentially could have
					// a RRectangle nearer than actualNearest
					if (tempDistance <= nearestDistance)
					{
		 ...
		 */
		private function recursiveIntersects( destiny:Array, r:RRectangle, n:RTreeNode ):void
		{
			for (var i:int = 0; i < n.entryCount; i++)
			{
				if (r.intersects(n.entries[i]))
				{
					...
					{
						var childNode:RTreeNode = this.getNode(n.ids[i]);
		 ...
		 */
		private var oldRectangle:RRectangle                        = new RRectangle(0, 0, 0, 0);

		private function condenseTree( l:RTreeNode ):void
		{
			// CT1 [Initialize] Set n=l. Set the list of eliminated
			// nodes to be empty.
			var n:RTreeNode             = l
			var parent:RTreeNode        = null
			...
			while (n.level != treeHeight)
			{
				...
				{
					// CT4 [Adjust covering RRectangle] If N has not been eliminated,
			...
			{
				var e:RTreeNode = this.getNode(eliminatedNodeIds.pop())
				for (var j:int = 0; j < e.entryCount; j++)
				{
					this.addRRectangle(e.entries[j], e.ids[j], e.level)
					e.entries[j] = null
				}
				e.entryCount = 0
			}
		...
		/**
		 *  Used by add(). Chooses a leaf to add the RRectangle to.
		 */
		private function chooseNode( r:RRectangle, level:int ):RTreeNode
		{
			// CL1 [Initialize] Set N to be the root node
			var n:RTreeNode = this.getNode(rootNodeId);
			this.parents = [];
			this.parentsEntry = [];
			...
			while (true)
			{
				...
				// whose RRectangle FI needs least enlargement to include EI. Resolve
				// ties by choosing the entry with the RRectangle of smaller volume.
				var leastEnlargement:Number = n.getEntry(0).enlargement(r)
				var index:int               = 0 // index of RRectangle in subtree
				for (var i:int = 1; i < n.entryCount; i++)
				{
					var tempRectangle:RRectangle = n.getEntry(i);
		 ...
		 */
		private function adjustTree( n:RTreeNode, nn:RTreeNode ):RTreeNode
		{
			...
			while (n.level != this.treeHeight)
			{
				...
				// AT3 [Adjust covering RRectangle in parent entry] Let P be the parent 
				// node of N, and let En be N's entry in P. Adjust EnI so that it tightly
				// encloses all entry rectangles in N.
				var parent:RTreeNode  = this.getNode(this.parents.pop())
		 ...
		 */
		private function checkConsistency( nodeId:String, expectedLevel:int, expectedMBR:RRectangle ):void
		{
			// go through the tree, and check that the internal data structures of 
			// the tree are not corrupted.    
			var n:RTreeNode              = this.getNode(nodeId)
			...
			{
				trace("Error: RTreeNode " + nodeId + ", expected level " + expectedLevel + ", actual level " + n.level);
			}

			var calculatedMBR:RRectangle = calculateMBR(n)

			if (!n.mbr.equals(calculatedMBR))
			{
				trace("Error: RTreeNode " + nodeId + ", calculated MBR does not equal stored MBR");
			}

			if (expectedMBR != null && !n.mbr.equals(expectedMBR))
			{
				trace("Error: RTreeNode " + nodeId + ", expected MBR (from parent) does not equal stored MBR");
			}

			// Check for corruption where a parent entry is the same object as the child MBR
			if (expectedMBR != null && n.mbr == (expectedMBR))
			{
				trace("Error: RTreeNode " + nodeId + " MBR using same RRectangle object as parent's entry");
			}
			...
			for (var i:int = 0; i < n.entryCount; i++)
			{
				if (n.entries[i] == null)
				{
					trace("Error: RTreeNode " + nodeId + ", Entry " + i + " is null");
				}
		 ...
		 */
		private function calculateMBR( n:RTreeNode ):RRectangle
		{
			var rect:RRectangle = n.entries[0]
			var mbr:RRectangle  = new RRectangle(rect.min[0], rect.min[1], rect.max[0], rect.max[1])
