package org.ash.core
{
	...
	internal class Family
	{
		...
		private var optionals:Dictionary;
		...
		private function init():void
		{
			...
			for each (var atom:XML in variables)
			{
				if (atom.@name != "entity" && atom.@name != "previous" && atom.@name != "next")
				{
					...
					optionals[componentClass] = atom.@name.toString().indexOf("$") != -1;
				}
			}
		}
		...
		internal function addIfMatch( entity:Entity ):void
		{
			if (!entities[entity])
			{
				...
				{
					if (!entity.has(componentClass) && !optionals[componentClass])
					{
						return;
					}
				}
		...
		{
			if (components[componentClass] && !optionals[componentClass])
			{
				remove(entity);
			}
		}
