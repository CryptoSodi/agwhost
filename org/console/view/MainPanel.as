...
package org.console.view
{
	...
	public class MainPanel extends ConsolePanel
	{
		...
		private function keyDownHandler( e:KeyboardEvent ):void
		{
			...
			if (e.keyCode == 18)
			{ //Keyboard.ALTERNATE not supported in flash 9
				_alt = true;
			}
		}
		...
		private function onScrollbarDown( e:MouseEvent ):void
		{
			...
			_scroller.addEventListener(MouseEvent.MOUSE_UP, onScrollBarUp, false, 0, true);
		}
		...
		private function onScrollBarUp( e:Event ):void
		{
			...
			_scroller.removeEventListener(MouseEvent.MOUSE_UP, onScrollBarUp);
		}
		//
		//
		...
		private function onScrollerDown( e:MouseEvent ):void
		{
			...
			_scroller.addEventListener(MouseEvent.MOUSE_UP, onScrollerUp, false, 0, true);
			e.stopPropagation();
		}
		...
		private function onScrollerUp( e:MouseEvent ):void
		{
			...
			_scroller.removeEventListener(MouseEvent.MOUSE_UP, onScrollerUp);
			_scrolling = false;
			onTraceScroll();
		}
		//
			...
			if (hints && hints.length)
			{
				...
				if (hints.length > 1)
				{
					...
					for (var i:int = 0; i < next.length; i++)
					{
						...
						{
							if (matched && _cmdField.text.length < i)
