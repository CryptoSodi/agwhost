package org.greensock
{
	import flash.display.*;
	import flash.events.*;
	import flash.utils.*;
	/**
	 * 	TweenManual is a super-lightweight (1.6k in AS3 and 2k in AS2) version of <a href="http://www.TweenLite.com">TweenLite</a>
	 ...
	 *  TweenManual can do everything TweenLite can do with the following exceptions:
	 * 	<ul>
	 * 		<li><b> No Plugins </b>- One of the great things about TweenLite is that you can activate
	 * 			plugins in order to add features (like autoAlpha, tint, blurFilter, etc.). TweenManual, however,
	 ...
	 * 			tweens can be much easier with TimelineLite and TimelineMax, but TweenManual instances cannot be inserted into
	 * 			TimelineLite or TimelineMax instances.</li>
	 *
	 * 		<li><b> Slight speed decrease </b>- Under very heavy loads, TweenManual won't perform quite as well as TweenLite, but
	 ...
	 * 			object (overwrite:true or overwrite:false) in TweenManual. TweenLite, however, can use OverwriteManager to expand
	 ...
	 * 			for details).</li>
	 *
	 * 		<li><b>Compared to TweenLite, TweenManual is missing the following methods/properties:</b>
	 * 			<ul>
	 * 				<li>pause()</li>
	 * 				<li>play()</li>
	 * 				<li>resume()</li>
	 ...
	 * 												   		TweenManual.to(mc, 1, {x:100, y:200});<br />
	 * 														TweenManual.to(mc, 1, {x:300, delay:2, overwrite:false}); //does NOT overwrite the previous tween.</code></li>
	 *
	 * 										<li><b> true (ALL_IMMEDIATE):</b> This is the default mode in TweenManual. All tweens of the same target
	 ...
	 * 												   		TweenManual.to(mc, 1, {x:100, y:200});<br />
	 * 														TweenManual.to(mc, 1, {x:300, delay:2, overwrite:true}); //immediately overwrites the previous tween</code></li>
	 * 										</ul></li>
	 * 	</ul>
	 *
	 * <b>EXAMPLES:</b> <br /><br />
	 ...
	 *
	 * <code>
	 * 		import org.greensock.TweenManual;<br /><br />
	 * 		TweenManual.to(mc, 1.5, {alpha:0.5, x:120});
	 * 	</code><br /><br />
	 *
	 ...
	 *
	 * 	<code>
	 * 		import org.greensock.TweenManual;<br />
	 * 		import org.greensock.easing.Back;<br /><br />
	 *
	 * 		TweenManual.to(mc, 5, {alpha:0.5, x:120, ease:Back.easeOut, delay:2, onComplete:onFinishTween, onCompleteParams:[5, mc]});<br />
	 ...
	 *  <code>
	 * 		import org.greensock.TweenManual;<br />
	 * 		import org.greensock.easing.Elastic;<br /><br />
	 *
	 * 		TweenManual.from(mc, 5, {y:"-100", ease:Elastic.easeOut});
	 *  </code><br /><br />
	 *
	 * <b>NOTES:</b><br /><br />
	 * <ul>
	 * 	<li> The base TweenManual class adds about 1.6k to your Flash file.</li>
	 *
	 * 	<li> Passing values as Strings will make the tween relative to the current value. For example, if you do
	 * 	  <code>TweenManual.to(mc, 2, {x:"-20"});</code> it'll move the mc.x to the left 20 pixels which is the same as doing
	 * 	  <code>TweenManual.to(mc, 2, {x:mc.x - 20});</code> You could also cast it like: <code>TweenManual.to(mc, 2, {x:String(myVariable)});</code></li>
	 *
	 * 	<li> Kill all tweens for a particular object anytime with the <code>TweenManual.killTweensOf(mc); </code></li>
	 *
	 * 	<li> You can kill all delayedCalls to a particular function using <code>TweenManual.killTweensOf(myFunction);</code>
	 ...
	 *
	 * 	<li> Use the <code>TweenManual.from()</code> method to animate things into place. For example, if you have things set up on
	...
	public class TweenManual
	{
		...
		/** @private **/
		protected static var _reservedProps:Object  = {ease:1, delay:1, overwrite:1, onComplete:1, onCompleteParams:1, runBackwards:1, immediateRender:1, onUpdate:1, onUpdateParams:1};
		...
		/** @private Easing method to use which determines how the values animate over time. Examples are Elastic.easeOut and Strong.easeIn. Many are found in the fl.motion.easing package or org.greensock.easing. **/
		 ...
		 */
		public function TweenManual( target:Object, duration:Number, vars:Object )
		{
			_time = 0;
			this.vars = vars;
		...
		public function renderTime( time:Number ):void
		{
			...
			{
				this.ratio = _ease(_time, 0, 1, this.duration);
			}
			while (--i > -1)
			{
				pt = _propTweens[i];
		...
		/**
		 * Static method for creating a TweenManual instance which can be more intuitive for some developers
		 ...
		 *
		 * var myTween:TweenManual = new TweenManual(mc, 1, {x:100}); <br />
		 * TweenManual.to(mc, 1, {x:100}); <br />
		 * var myTween:TweenManual = TweenManual.to(mc, 1, {x:100});</code>
		 *
		 * @param target Target object whose properties this tween affects. This can be ANY object, not just a DisplayObject.
		 * @param duration Duration in seconds
		 * @param vars An object containing the end values of the properties you're tweening, like {x:100, y:50}. It can also contain special properties like "onComplete", "ease", "delay", etc.
		 * @return TweenManual instance
		 */
		public static function to( target:Object, duration:Number, vars:Object ):TweenManual
		{
			return new TweenManual(target, duration, vars);
		}

		/**
		 * Static method for creating a TweenManual instance that tweens in the opposite direction
		 * compared to a TweenManual.to() tween. In other words, you define the START values in the
		 ...
		 * because you can build them in their end positions and do some simple TweenManual.from()
		 ...
		 *
		 * TweenManual.from(mc, 1.5, {alpha:0, delay:2});</code>
		 *
		 * @param target Target object whose properties this tween affects. This can be ANY object, not just a DisplayObject.
		 * @param duration Duration in seconds
		 ...
		 * @return TweenManual instance
		 */
		public static function from( target:Object, duration:Number, vars:Object ):TweenManual
		{
		 ...
		 *
		 * TweenManual.delayedCall(1, myFunction, ["param1", 2]); <br />
		 * function myFunction(param1:String, param2:Number):void { <br />
		 *     trace("called myFunction and passed params: " + param1 + ", " + param2); <br />
		 * } </code>
		 *
		 * @param delay Delay in seconds before the function should be called
		 * @param onComplete Function to call
		 * @param onCompleteParams An Array of parameters to pass the function.
		 * @return TweenManual instance
		 */
		public static function delayedCall( delay:Number, onComplete:Function, onCompleteParams:Array = null ):TweenManual
		{
			return new TweenManual(onComplete, 0, {delay:delay, onComplete:onComplete, onCompleteParams:onCompleteParams, overwrite:0});
		}
		 ...
		 */
		public static function updateAll( time:Number ):void
		{
			var ml:Dictionary = _masterList, a:Array, tgt:Object, i:int, t:Number, tween:TweenManual;
			for (tgt in ml)
			{
				a = ml[tgt];
				i = a.length;
		...
		public static function killTweensOf( target:Object, complete:Boolean = false ):void
		{
			if (target in _masterList)
			{
				if (complete)
				{
					...
					while (--i > -1)
					{
						if (!TweenManual(a[i]).gc)
						{
							TweenManual(a[i]).complete(false);
						}
					}
				}
